...
pub fn tokenize<'s, 'v>(input: &'s str, lines: &'v mut Vec<&'s str>) -> impl Iterator<Item = Token<'s>> + 'v {
    let mut cursor = Cursor::new(input, lines);
...
mod tests {
    ...
    fn check_lexing<'a>(src: &'a str, expect: &str) {
