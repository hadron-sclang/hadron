...
void JSONTransport::JSONTransportImpl::serializeParseNode(hadron::ThreadContext* context,
    ...
    case hadron::parse::NodeType::kArrayRead: {
        const auto arrayRead = reinterpret_cast<const hadron::parse::ArrayReadNode*>(node);
        jsonNode.AddMember("nodeType", rapidjson::Value("ArrayRead"), document.GetAllocator());

        path.emplace_back(makeToken("targetArray"));
        serializeParseNode(context, arrayRead->targetArray.get(), document, path, serial);
        path.pop_back();

        path.emplace_back(makeToken("indexArgument"));
        serializeParseNode(context, arrayRead->indexArgument.get(), document, path, serial);
        path.pop_back();
    } break;

    case hadron::parse::NodeType::kArrayWrite: {
        const auto arrayWrite = reinterpret_cast<const hadron::parse::ArrayWriteNode*>(node);
        jsonNode.AddMember("nodeType", rapidjson::Value("ArrayWrite"), document.GetAllocator());

        path.emplace_back(makeToken("targetArray"));
        serializeParseNode(context, arrayWrite->targetArray.get(), document, path, serial);
        path.pop_back();

        path.emplace_back(makeToken("indexArgument"));
        serializeParseNode(context, arrayWrite->indexArgument.get(), document, path, serial);
        path.pop_back();

        path.emplace_back(makeToken("value"));
        serializeParseNode(context, arrayWrite->value.get(), document, path, serial);
        path.pop_back();
    } break;
    ...
    case hadron::parse::NodeType::kNew: {
        const auto newNode = reinterpret_cast<const hadron::parse::NewNode*>(node);
        jsonNode.AddMember("nodeType", rapidjson::Value("New"), document.GetAllocator());

        path.emplace_back(makeToken("target"));
        serializeParseNode(context, newNode->target.get(), document, path, serial);
        path.pop_back();

        path.emplace_back(makeToken("arguments"));
        serializeParseNode(context, newNode->arguments.get(), document, path, serial);
        path.pop_back();

        path.emplace_back(makeToken("keywordArguments"));
        serializeParseNode(context, newNode->keywordArguments.get(), document, path, serial);
        path.pop_back();
    } break;
...
void JSONTransport::JSONTransportImpl::serializeSlot(hadron::ThreadContext* context, hadron::Slot slot,
    ...
    case hadron::TypeFlags::kCharFlag:
        target.AddMember("type", rapidjson::Value("char"), document.GetAllocator());
        target.AddMember("value", rapidjson::Value(slot.getChar()), document.GetAllocator());
        break;
    case hadron::TypeFlags::kObjectFlag:
...
void JSONTransport::JSONTransportImpl::serializeHIR(hadron::ThreadContext* context, const hadron::hir::HIR* hir,
    ...
    case hadron::hir::Opcode::kLoadOuterFrame: {
    ...
    case hadron::hir::Opcode::kReadFromClass: {
        const auto readClass = reinterpret_cast<const hadron::hir::ReadFromClassHIR*>(hir);
        jsonHIR.AddMember("opcode", "ReadFromClass", document.GetAllocator());

        rapidjson::Value classVariableArray;
        serializeValue(context, readClass->classVariableArray, frame, classVariableArray, document);
        jsonHIR.AddMember("classVariableArray", classVariableArray, document.GetAllocator());

        jsonHIR.AddMember("arrayIndex", rapidjson::Value(readClass->arrayIndex), document.GetAllocator());

        rapidjson::Value valueName;
        serializeSymbol(context, readClass->valueName, valueName, document);
    ...
    case hadron::hir::Opcode::kReadFromThis: {
        const auto readThis = reinterpret_cast<const hadron::hir::ReadFromThisHIR*>(hir);
        jsonHIR.AddMember("opcode", "ReadFromThis", document.GetAllocator());

        rapidjson::Value thisId;
        serializeValue(context, readThis->thisId, frame, thisId, document);
        jsonHIR.AddMember("thisId", thisId, document.GetAllocator());

        jsonHIR.AddMember("index", rapidjson::Value(readThis->index), document.GetAllocator());

        rapidjson::Value valueName;
        serializeSymbol(context, readThis->valueName, valueName, document);
        jsonHIR.AddMember("valueName", valueName, document.GetAllocator());
    } break;
    ...
    case hadron::hir::Opcode::kWriteToClass: {
        const auto writeClass = reinterpret_cast<const hadron::hir::WriteToClassHIR*>(hir);
        jsonHIR.AddMember("opcode", "WriteClass", document.GetAllocator());

        rapidjson::Value classVariableArray;
        serializeValue(context, writeClass->classVariableArray, frame, classVariableArray, document);
        jsonHIR.AddMember("classVariableArray", classVariableArray, document.GetAllocator());

        jsonHIR.AddMember("arrayIndex", rapidjson::Value(writeClass->arrayIndex), document.GetAllocator());

        rapidjson::Value valueName;
        serializeSymbol(context, writeClass->valueName, valueName, document);
        jsonHIR.AddMember("valueName", valueName, document.GetAllocator());

        rapidjson::Value toWrite;
        serializeValue(context, writeClass->toWrite, frame, toWrite, document);
        jsonHIR.AddMember("toWrite", toWrite, document.GetAllocator());
    } break;

    case hadron::hir::Opcode::kWriteToFrame: {
        const auto writeFrame = reinterpret_cast<const hadron::hir::WriteToFrameHIR*>(hir);
        ...
        rapidjson::Value toWrite;
        serializeValue(context, writeFrame->toWrite, frame, toWrite, document);
        jsonHIR.AddMember("toWrite", toWrite, document.GetAllocator());
    } break;

    case hadron::hir::Opcode::kWriteToThis: {
        const auto writeThis = reinterpret_cast<const hadron::hir::WriteToThisHIR*>(hir);
        jsonHIR.AddMember("opcode", "WriteToThis", document.GetAllocator());

        rapidjson::Value thisId;
        serializeValue(context, writeThis->thisId, frame, thisId, document);
        jsonHIR.AddMember("thisId", thisId, document.GetAllocator());

        jsonHIR.AddMember("index", rapidjson::Value(writeThis->index), document.GetAllocator());

        rapidjson::Value valueName;
        serializeSymbol(context, writeThis->valueName, valueName, document);
        jsonHIR.AddMember("valueName", valueName, document.GetAllocator());

        rapidjson::Value toWrite;
        serializeValue(context, writeThis->toWrite, frame, toWrite, document);
        jsonHIR.AddMember("toWrite", toWrite, document.GetAllocator());
    } break;
...
void JSONTransport::JSONTransportImpl::serializeLIR(hadron::ThreadContext* context, const hadron::lir::LIR* lir,
    ...
    case hadron::lir::Opcode::kInterrupt: {
        const auto interrupt = reinterpret_cast<const hadron::lir::InterruptLIR*>(lir);
        jsonLIR.AddMember("opcode", "Interrupt", document.GetAllocator());
        jsonLIR.AddMember("interruptCode", rapidjson::Value(interrupt->interruptCode), document.GetAllocator());
    } break;
