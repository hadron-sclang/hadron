...
library::LinearFrame BlockSerializer::serialize(ThreadContext* context, const library::CFGFrame frame) {
    ...
    for (int32_t i = 0; i < blockOrder.size(); ++i) {
        ...
        auto predecessors =
            library::TypedArray<library::LabelId>::typedArrayAlloc(context, block.predecessors().size());
        ...
        auto successors = library::TypedArray<library::LabelId>::typedArrayAlloc(context, block.successors().size());
...
void BlockSerializer::lower(ThreadContext* context, library::HIR hir, library::LinearFrame linearFrame,
    ...
    case library::BlockLiteralHIR::nameHash(): {
        ...
        auto functionSymbolVReg = linearFrame.append(
            context, library::HIRId(),
            library::LoadConstantLIR::makeLoadConstant(context, context->symbolTable->functionSymbol().slot()).toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(context, library::kStackPointerVReg,
        ...
        linearFrame.append(
            context, library::HIRId(),
            library::InterruptLIR::makeInterrupt(context, ThreadContext::InterruptCode::kNewObject).toBase(),
            instructions);

        auto functionVRegTagged =
            linearFrame.append(context, blockLiteralHIR.id(),
                               library::LoadFromPointerLIR::makeLoadFromPointer(
                                   context, library::kStackPointerVReg, offsetof(schema::FramePrivateSchema, arg0))
                                   .toBase(),
                               instructions);
        auto functionVReg = linearFrame.append(
            context, library::HIRId(), library::RemoveTagLIR::makeRemoveTagLIR(context, functionVRegTagged).toBase(),
            instructions);

        // Set the Function context to the current context pointer.
        // TODO: Add the pointer flagging
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(context, functionVReg,
                                                                          offsetof(schema::FunctionSchema, context),
                                                                          library::kContextPointerVReg)
                               .toBase(),
                           instructions);

        // Load the functiondef/method into a register.
        auto functionDefVReg = linearFrame.append(
            context, library::HIRId(),
            library::LoadConstantLIR::makeLoadConstant(context, blockLiteralHIR.functionDef().slot()).toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(
                               context, functionVReg, offsetof(schema::FunctionSchema, def), functionDefVReg)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::BranchHIR::nameHash(): {
        ...
        linearFrame.append(context, library::HIRId(),
                           library::BranchLIR::makeBranch(context, branchHIR.blockId()).toBase(), instructions);
    } break;

    case library::BranchIfTrueHIR::nameHash(): {
        auto branchIfTrueHIR = library::BranchIfTrueHIR(hir.slot());
        linearFrame.append(context, library::HIRId(),
                           library::BranchIfTrueLIR::makeBranchIfTrue(
                               context, linearFrame.hirToReg(branchIfTrueHIR.condition()), branchIfTrueHIR.blockId())
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::ConstantHIR::nameHash(): {
        ...
        linearFrame.append(context, constantHIR.id(),
                           library::LoadConstantLIR::makeLoadConstant(context, constantHIR.constant()).toBase(),
                           instructions);
    } break;

    case library::LoadOuterFrameHIR::nameHash(): {
        auto loadOuterFrameHIR = library::LoadOuterFrameHIR(hir.slot());
        auto innerContextVReg = loadOuterFrameHIR.innerContext()
            ? linearFrame.hirToReg(loadOuterFrameHIR.innerContext())
            : library::kFramePointerVReg;
        linearFrame.append(context, loadOuterFrameHIR.id(),
                           library::LoadFromPointerLIR::makeLoadFromPointer(
                               context, innerContextVReg, offsetof(schema::FramePrivateSchema, context))
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::MessageHIR::nameHash(): {
        ...
        auto selectorVReg = linearFrame.append(
            context, library::HIRId(),
            library::LoadConstantLIR::makeLoadConstant(context, messageHIR.selector(context).slot()).toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(context, library::kStackPointerVReg,
                                                                          offsetof(schema::FramePrivateSchema, method),
                                                                          selectorVReg)
                               .toBase(),
                           instructions);
        ...
        auto numberOfArgsVReg = linearFrame.append(
            context, library::HIRId(),
            library::LoadConstantLIR::makeLoadConstant(context, Slot::makeInt32(messageHIR.arguments().size()))
                .toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(context, library::kStackPointerVReg,
        ...
        auto numberOfKeyArgsVReg = linearFrame.append(
            context, library::HIRId(),
            library::LoadConstantLIR::makeLoadConstant(context, Slot::makeInt32(messageHIR.keywordArguments().size()))
                .toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(
                               context, library::kStackPointerVReg, offsetof(schema::FramePrivateSchema, homeContext),
                               numberOfKeyArgsVReg)
                               .toBase(),
                           instructions);
        ...
        for (int32_t i = 0; i < messageHIR.arguments().size(); ++i) {
            ...
            linearFrame.append(
                context, library::HIRId(),
                library::StoreToPointerLIR::makeStoreToPointer(context, library::kStackPointerVReg, offset, argVReg)
                    .toBase(),
                instructions);
            offset += kSlotSize;
        }
        ...
        linearFrame.append(
            context, library::HIRId(),
            library::InterruptLIR::makeInterrupt(context, ThreadContext::InterruptCode::kDispatch).toBase(),
            instructions);

        // Load return value.
        linearFrame.append(context, messageHIR.id(),
    ...
    case library::MethodReturnHIR::nameHash(): {
        ...
        linearFrame.append(context, library::HIRId(), library::PopFrameLIR::make(context).toBase(), instructions);
        auto returnAddressTagged =
            linearFrame.append(context, library::HIRId(),
                               library::LoadFromPointerLIR::makeLoadFromPointer(
                                   context, library::kFramePointerVReg, offsetof(schema::FramePrivateSchema, ip))
                                   .toBase(),
                               instructions);
        auto returnAddress = linearFrame.append(
            context, library::HIRId(), library::RemoveTagLIR::makeRemoveTagLIR(context, returnAddressTagged).toBase(),
            instructions);
        linearFrame.append(context, library::HIRId(),
    ...
    case library::ReadFromClassHIR::nameHash(): {
        ...
        linearFrame.append(context, readFromClassHIR.id(),
                           library::LoadFromPointerLIR::makeLoadFromPointer(context, classVarVReg,
                                                                            readFromClassHIR.arrayIndex() * kSlotSize)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::ReadFromContextHIR::nameHash(): {
        ...
        linearFrame.append(context, readFromContextHIR.id(),
                           library::LoadFromPointerLIR::makeLoadFromPointer(context, library::kContextPointerVReg,
                                                                            readFromContextHIR.offset() * kSlotSize)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::ReadFromFrameHIR::nameHash(): {
        ...
        auto frameVReg =
            readFromFrameHIR.frameId() ? linearFrame.hirToReg(readFromFrameHIR.frameId()) : library::kFramePointerVReg;
        linearFrame.append(context, readFromFrameHIR.id(),
                           library::LoadFromPointerLIR::makeLoadFromPointer(context, frameVReg,
                                                                            readFromFrameHIR.frameIndex() * kSlotSize)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::ReadFromThisHIR::nameHash(): {
        ...
        linearFrame.append(
            context, readFromThisHIR.id(),
            library::LoadFromPointerLIR::makeLoadFromPointer(context, thisVReg, readFromThisHIR.index() * kSlotSize)
                .toBase(),
            instructions);
    } break;
    ...
    case library::StoreReturnHIR::nameHash(): {
        ...
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(context, library::kFramePointerVReg,
                                                                          offsetof(schema::FramePrivateSchema, arg0),
                                                                          returnVReg)
                               .toBase(),
                           instructions);
    ...
    case library::WriteToClassHIR::nameHash(): {
        ...
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(
                               context, classVarVReg, writeToClassHIR.arrayIndex() * kSlotSize, toWriteVReg)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::WriteToFrameHIR::nameHash(): {
        ...
        auto frameVReg =
            writeToFrameHIR.frameId() ? linearFrame.hirToReg(writeToFrameHIR.frameId()) : library::kFramePointerVReg;
        auto toWriteVReg = linearFrame.hirToReg(writeToFrameHIR.toWrite());
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(
                               context, frameVReg, writeToFrameHIR.frameIndex() * kSlotSize, toWriteVReg)
                               .toBase(),
                           instructions);
    } break;
    ...
    case library::WriteToThisHIR::nameHash(): {
        ...
        linearFrame.append(context, library::HIRId(),
                           library::StoreToPointerLIR::makeStoreToPointer(
                               context, thisVReg, writeToThisHIR.index() * kSlotSize, toWriteVReg)
                               .toBase(),
                           instructions);
    } break;
