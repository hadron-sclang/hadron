...
impl<'s> DiagnosticLocationTranslator<'s, Token<'s>> for TokenizedBuffer<'s> {
    fn get_location(&self, token: &Token<'s>) -> DiagnosticLocation<'s> {
        ...
        DiagnosticLocation { file_name: self.source.file_name(), line_number: token.line,
                column_number: token.column, line: self.lines[line_index] }
    }
}
...
mod tests {
    ...
    #[test]
    fn smoke_test() {
        check_lexing(sclang!(r#"SynthDef(\a, { var snd = SinOsc.ar(440, mul: 0.5); snd; });"#), vec![
...
Token { kind: Binop { kind: BinopIdentifier }, string: "+//*", line: 1, column: 1 }]);
    ...
    fn block_comments() {
        ...
        check_lexing(sclang!("/**/ /*/ hello */"), vec![
Token { kind: Ignored { kind: BlockComment }, string: "/**/", line: 1, column: 1 },
Token { kind: Ignored { kind: BlankSpace }, string: " ", line: 1, column: 5 },
Token { kind: Ignored { kind: BlockComment}, string: "/*/ hello */", line: 1, column: 6 }]);

        // Commented out code.
        check_lexing(sclang!(r#"/* var a = 2; */"#), vec![
Token { kind: Ignored { kind: BlockComment }, string: "/* var a = 2; */", line: 1, column: 1 }]);
...
Token { kind: Character { is_escaped: false }, string: "$\"", line: 1, column: 10 }]);
    ...
    #[test]
    fn class_names() {
        check_lexing(sclang!("A B C SinOsc Main_32a"), vec![
...
Token { kind: Ellipses, string: "...", line: 1, column: 24 }]);
    ...
    #[test]
    fn identifiers() {
        check_lexing(sclang!("a b c if while const classvar zz_Top3"), vec![
...
Token { kind: Identifier, string: "zz_Top3", line: 1, column: 31 }]);
        ...
        check_lexing(sclang!("mul: add:"), vec![
...
Token { kind: Keyword, string: "add:", line: 1, column: 6 }]);
    ...
    #[test]
    fn inline_symbols() {
        check_lexing(sclang!(r#"\a \b \c \1 \ \_ \A_l0ng3r_SYMBOL"#), vec![
...
// /* start of block
        // end of line */"#), vec![
Token { kind: Ignored { kind: LineComment }, string: "// start of line\n", line: 1, column: 1 },
Token { kind: Ignored { kind: LineComment }, string: "// /* start of block\n", line: 2, column: 1 },
Token { kind: Ignored { kind: BlankSpace }, string: "        ", line: 3, column: 1 },
Token { kind: Ignored { kind: LineComment }, string: "// end of line */", line: 3, column: 9 }]);
    }

    #[test]
    fn numbers() {
        check_lexing(sclang!(r#"123 0x1aAfF 36rZIGZAG10
100.100 4bbb 23s200 16rA.F 1.7e-9 1e+7"#), vec![
...
Token { kind: Number { kind: FloatSci }, string: "1e+7", line: 2, column: 35 }]);
    ...
    #[test]
    fn primitives() {
        check_lexing(sclang!("_abc123 _Test_ _PrimitiTooTaah"), vec![
